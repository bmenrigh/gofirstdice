...
uint64_t min_tallie_left[SIDES][DICE];
uint64_t max_tallie_left[SIDES][DICE];
...
void sum_find(int (*g)[DICE][SIDES], uint64_t (*t)[DICE][DICE], int depth) {
  ...
  if (((MIRRORSYM == 0) && (depth < SIDES)) ||
    ...
    for (d = 0; d < DICE; d++) {
      for (pl = 0; pl < DICE; pl++) {
	if ((*t)[d][pl] + max_tallie_left[depth][pl] < GOALTALLY) {
	  /*fprintf(stderr, "Abandoning (under min) at depth %d\n", depth);*/
	  return;
	}
	if ((*t)[d][pl] + min_tallie_left[depth][pl] > GOALTALLY) {
...
void sum_find_row(int (*g)[DICE][SIDES], uint64_t (*t)[DICE][DICE],
  ...
  if (((MIRRORSYM == 0) && (depth < SIDES)) ||
      ((MIRRORSYM == 1) && (depth < (SIDES / 2)))) {
    for (pl = 0; pl < DICE; pl++) {
      if ((*t)[row][pl] + max_tallie_left[depth][pl] < GOALTALLY) {
	/*fprintf(stderr, "Abandoning (under min) at depth %d, row %d\n",
	  depth, row);*/
	return;
      }
      if ((*t)[row][pl] + min_tallie_left[depth][pl] > GOALTALLY) {
...
int compare_datum(const void *a, const void *b, __attribute__((__unused__)) void *param) {
...
void build_tallies_left() {
  ...
  for (p = 0; p < DICE; p++) {
	...
	if (t > max) {
      ...
      if (s == l) {
	min_tallie_left[s][p] = min;
	max_tallie_left[s][p] = max;
      }
      else {
	min_tallie_left[s][p] = min + min_tallie_left[s + 1][p];
	max_tallie_left[s][p] = max + max_tallie_left[s + 1][p];
      }
