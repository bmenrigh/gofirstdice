...
#define THREADABORT 0
#define THREADLIMIT 1000000000
...
#define SUBSETMIN  2 /* Minimum size of subsets */
...
uint64_t stscore[DICE][DICE][DICE];
uint64_t t_cache_row[DICE][SIDES][DICE][DICE];
uint64_t st_cache_row[DICE][SIDES][DICE][DICE][DICE];

int selection[DICE];
uint64_t tallies[DICE][DICE];
uint64_t subset_tallies[DICE + 1][DICE][DICE];
uint64_t tallie_cache[DICE * SIDES][DICE];
int64_t subset_tallie_cache[DICE + 1][DICE][SIDES][DICE][DICE];
uint64_t min_tallie_left[DICE][SIDES][DICE];
uint64_t max_tallie_left[DICE][SIDES][DICE];
uint64_t subset_min_tallie_left[DICE][DICE][SIDES][DICE];
uint64_t subset_max_tallie_left[DICE][DICE][SIDES][DICE];
...
void build_subset_tallie_table_math(int (*)[DICE][SIDES], int);
void build_tallies_left(int (*)[DICE][SIDES],
			uint64_t (*)[DICE][SIDES][DICE],
			uint64_t (*)[DICE][SIDES][DICE], int);
void build_subset_tallies_left(int (*)[DICE][SIDES],
			       uint64_t (*)[DICE][DICE][SIDES][DICE],
			       uint64_t (*)[DICE][DICE][SIDES][DICE],
			       int64_t (*)[DICE + 1][DICE][SIDES][DICE][DICE],
			       int);
int subset_n_places_fair(int (*)[DICE][SIDES], int);
void sum_find_row(int (*)[DICE][SIDES], uint64_t (*)[DICE][DICE],
		  uint64_t (*)[DICE][DICE][DICE], int, int);
...
int main(void) {
  ...
  __attribute__((__unused__)) int64_t temptallie;
  __attribute__((__unused__)) uint64_t debugtallie;
  __attribute__((__unused__)) int d, s, p, v;
  ...
  /* setup the subset goals */
  for (i = 0; i < DICE; i++) {
    subset_goal[i] = ((uint64_t)pow(SIDES, i + 1)) / (i + 1);
  }  
  ...
  build_tallies_left(&grid, &min_tallie_left, &max_tallie_left, 0);

  /* the perm stuff */
  build_perm_stuff();


  /* Print out the tallie table cache */
  /*for (s = 0; s < SIDES; s++) {
    for (d = 0; d < DICE; d++) {
      v = (s * DICE) + d;
      for (p = 0; p < DICE; p++) {
	printf("$PL[%d][%d][%d] = %lu;\n", s, d, p, tallie_cache[v][p]);
      }
    }
  }
  exit(0);*/
  ...
  /* Debug stuff for subset tallie crap */
  /*build_subset_tallie_table_math(&grid, 2);
  build_subset_tallies_left(&grid,
			    &subset_min_tallie_left,
			    &subset_max_tallie_left,
			    &subset_tallie_cache,
			    2);
  ...
  debugtallie = 0;
  for (i = 0; i < SIDES; i++) {
    printf("Min, max tallies left for s=%d, min=%ld, max=%ld\n",
	   i, subset_min_tallie_left[2][i][1],
	   subset_max_tallie_left[2][i][1]);
	   
    temptallie = subset_tallie_cache[2][grid[2][i] % DICE][i][grid[2][i] % DICE][1];
    printf("addding in %ld for s=%d; num=%d\n", temptallie, i, grid[2][i]);
    debugtallie += temptallie;
  }
  printf("%lu\n", debugtallie);
  ...
  memset(&subset_tallies, 0, sizeof(subset_tallies));
  ...
  /* Make sure we aren't trying to mirror more than is possible */
  if ((MIRRORSYM == 1) && (MIRRFIRSTN > (SIDES / 2))) {
    fprintf(stderr, "You can't mirror more than the first half!\n");
    exit(1);
  }  

  /* Pair + mirror doesn't work yet */
  if ((PAIRSYM == 1) && (MIRRORSYM == 1)) {
    fprintf(stderr, "Pair + Mirror Symmetry not implemented yet!\n");
    exit(1);
  }  
  
  memset(&tscore, 0, sizeof(tscore));
  memset(&stscore, 0, sizeof(stscore));
  /*sum_find(&grid, &tscore, 0);*/
  sum_find_row(&grid, &tscore, &stscore, 0, 0);
...
void sum_find_row(int (*g)[DICE][SIDES], uint64_t (*t)[DICE][DICE],
		  uint64_t (*st)[DICE][DICE][DICE],
		  int row, int depth) {

  int d, pl, i, tmpd, tmpdr, rnum;
  uint64_t pr_score;
  char gstr[(DICE * SIDES) + 1];

  /* We gotta build our tallie left cache for this row */
  if ((depth == 0) && (row != DICE)) {
    build_tallies_left(g, &min_tallie_left, &max_tallie_left, row);
  }

  if ((depth == 0) && (row >= (SUBSETMIN - 1)) && (row <= (DICE - 1))) {
  ...
  if (((depth < SIDES)) &&
    ...
    if ((row >= (SUBSETMIN - 1)) && (row <= (DICE - 1))) {
      
      if (depth == 0) {
	build_subset_tallie_table_math(g, row);
	build_subset_tallies_left(g,
				  &subset_min_tallie_left,
				  &subset_max_tallie_left,
				  &subset_tallie_cache,
				  row);
      ...
      for (rnum = 0; rnum <= row; rnum++) {
	for (pl = 0; pl <= row; pl++) { /* n rows means n places */
	
	  if ((*st)[row][rnum][pl] +
	      subset_max_tallie_left[row][rnum][depth][pl] <
	      subset_goal[row]) {
	    /*fprintf(stderr, "Max below goal at row=%d, rnum=%d, depth=%d, "
		    "place=%d, max=%lu, goal=%lu\n", row, rnum, depth, pl,
		    (*st)[row][rnum][pl] +
		    subset_max_tallie_left[row][rnum][depth][pl],
		    subset_goal[row]);*/
	    return;
	    ...
	    /*fprintf(stderr, "Max in range at row=%d, rnum=%d, depth=%d, "
		    "place=%d, max=%lu, goal=%lu\n", row, rnum, depth, pl,
		    (*st)[row][rnum][pl] +
		    subset_max_tallie_left[row][rnum][depth][pl],
		    subset_goal[row]);*/
	  ...
	  if ((*st)[row][rnum][pl] +
	      subset_min_tallie_left[row][rnum][depth][pl]
	      > subset_goal[row]) {
	    /*fprintf(stderr, "Min above goal at row=%d, rnum=%d, depth=%d, "
		    "place=%d, min=%lu, goal=%lu\n", row, rnum, depth, pl,
		    (*st)[row][rnum][pl] +
		    subset_min_tallie_left[row][rnum][depth][pl],
		    subset_goal[row]);*/
	    return;
	    ...
	    /*fprintf(stderr, "Min in range at row=%d, rnum=%d, depth=%d, "
		    "place=%d, min=%lu, goal=%lu\n", row, rnum, depth, pl,
		    (*st)[row][rnum][pl] +
		    subset_min_tallie_left[row][rnum][depth][pl],
		    subset_goal[row]);*/
    ...
    } /* end if row meets the subset minimum */
  } /* end mirror check */
  ...
  if (depth >= (SIDES)) {
	...
	fprintf(stderr, "\n");*/
      ...
      memset(&(t_cache_row[row + 1][0]), 0, sizeof(tscore));
      memset(&(st_cache_row[row + 1][0]), 0, sizeof(stscore));
      
      /*fprintf(stderr, "Moving on to row %d\n", row + 1);*/
      sum_find_row(g, &(t_cache_row[row + 1][0]),
		   &(st_cache_row[row + 1][0]), row + 1, 0);
      return;
    }
  ...
  if (MIRRORSYM == 1) {
	...
	((DICE * SIDES) - 1) - (*g)[row][(SIDES - 1) - depth];
      ...
      memcpy(&(t_cache_row[row][depth]), t, sizeof(tscore));
      memcpy(&(st_cache_row[row][depth]), st, sizeof(stscore));
      /* compute new tallie */
      /*for (pl = 0; pl < DICE; pl++) {
	t_cache_row[row][depth][row][pl] = (*t)[row][pl] +
      ...
      sum_find_row(&(g_cache_row[row][depth]), &(t_cache_row[row][depth]),
		   &(st_cache_row[row][depth]),
		   row, depth + 1);
      return;
    }
  } /* end if mirror sym */

  /* Just assign this row a value if we're doing pair symmetry */
  if (PAIRSYM == 1) {
    if ((depth & 1) == 1) {
      ...
      /* assing this the value based on the prev col */
      tmpd = (*g)[row][depth - 1] % DICE;
      g_cache_row[row][depth][row][depth] =
	(DICE * depth) + ((DICE - 1) - tmpd);

      /*fprintf(stderr, "depth: %d, prevd: %d, newd: %d\n", depth,
	      (*g)[row][depth - 1],
	      g_cache_row[row][depth][row][depth]);*/
  ...
  for (d = row; d < DICE; d++) {
	...
	t_cache_row[row][depth][row][pl] += 
      ...
      if ((PAIRSYM == 1) && ((depth & 1) == 0)) {
	tmpd = g_cache_row[row][depth][row][depth] % DICE;
	t_cache_row[row][depth][row][pl] += 
	  tallie_cache[(DICE * (depth + 1)) + ((DICE - 1) - tmpd)][pl];
      }	
    ...
    if ((row >= (SUBSETMIN - 1)) && (row <= (DICE - 1))) {
      ...
      for (rnum = 0; rnum <= row; rnum++) {
	tmpdr = g_cache_row[row][depth][rnum][depth] % DICE;
	
	for (pl = 0; pl <= row; pl++) { /* n rows, n places */
	  
	  st_cache_row[row][depth][row][rnum][pl] = (*st)[row][rnum][pl] +
	    subset_tallie_cache[row][tmpdr][depth][tmpd][pl];
	  
	  if ((MIRRORSYM == 1) && (depth < MIRRFIRSTN)) {
	    st_cache_row[row][depth][row][rnum][pl] +=
	      subset_tallie_cache[row][(DICE - 1) - tmpdr]
	      [(SIDES - 1) - depth][(DICE - 1) - tmpd][pl];
	  }

	  if ((PAIRSYM == 1) && ((depth & 1) == 0)) {
	    st_cache_row[row][depth][row][rnum][pl] +=
	      subset_tallie_cache[row][(DICE - 1) - tmpdr]
	      [depth + 1][(DICE - 1) - tmpd][pl];
	  }
	}
      }
    }
  ...
  if (((DOFORK == 1) && (row == 0) && (depth == FORKDEPTH)) ||
      ((THREADABORT == 1) && (threadcount > THREADLIMIT) &&
       (depth >= FORKDEPTH))) {
    semwait(0, __LINE__);
    shmdata->count_pl_fair += t_count_pl_fair;
...
void build_subset_tallie_table_math(int (*g)[DICE][SIDES], int n) {

  int d, rd, pd, s, p, ab, faked, skip, i, dn;

  dn = n + 1;
  
  /* col num % DICE; side; nth place choice; place */
  /* int64_t subset_tallie_cache[DICE][SIDES][DICE][DICE]; */
  for (d = 0; d < DICE; d++) {
    for (s = 0; s < SIDES; s++) {
      for (pd = 0; pd < DICE; pd++) {
	for (p = 0; p < DICE; p++) {
	  subset_tallie_cache[n][d][s][pd][p] = -1;
	}
      }
    }
  }
  ...
  for (s = 0; s < SIDES; s++) {
    for (d = 0; d < DICE; d++) {
      for (pd = 0; pd < DICE; pd++) { /* possible d choice for nth row */
	...
	for (p = 0; p <= n; p++) {
	  ...
	  for (ab = 0; ((ab <= p) && (ab <= faked)); ab++) {
	    if (subset_tallie_cache[n][d][s][pd][p] == -1) {
	      subset_tallie_cache[n][d][s][pd][p] = 0;
	    }
	    
	    subset_tallie_cache[n][d][s][pd][p] +=
	      ((choose(faked, ab) * /* Rows above d */
		(uint64_t)pow((SIDES - s) - 1, ab)) * /* Sides to the right */
	       (choose((dn - 1) - faked, p - ab) * /* Rows below d */
		(uint64_t)pow(SIDES - s, p - ab)) * /* Sides to the right */
	       (uint64_t)pow(s + 1, faked - ab) * /* combos (above / left) */
	       (uint64_t)pow(s, ((dn - faked) - 1) - (p - ab)));
	  }
...
void build_tallies_left(int (*g)[DICE][SIDES],
  ...
  for (p = 0; p < DICE; p++) { /* for each place */
	...
	if (s == (l - 1)) {
	  ...
	  (*mint)[rows][s][p] = min + (*mint)[rows][s + 2][p];
	  (*maxt)[rows][s][p] = max + (*maxt)[rows][s + 2][p];
	}
      }
...
void build_subset_tallies_left(int (*g)[DICE][SIDES],
			       uint64_t (*minst)[DICE][DICE][SIDES][DICE],
			       uint64_t (*maxst)[DICE][DICE][SIDES][DICE],
			       int64_t (*stc)[DICE + 1]
			       [DICE][SIDES][DICE][DICE],
			       int rows) {

  int d, s, p, vd, pd, l, rnum;

  int64_t min[DICE], max[DICE], t, tempt;
  ...
  for (p = 0; p <= rows; p++) { /* for each place */
    for (s = l; s >= 0; s--) { /* start from last to first */
	...
	min[rnum] = (uint64_t)pow(SIDES, DICE);
	max[rnum] = 0;
      }
	...
	for (pd = 0; pd < DICE; pd++) {
	  ...
	  if (MIRRORSYM == 1) {
	    if (s < MIRRFIRSTN) {
	      ...
	      tempt = (*stc)[rows][(DICE - 1) - vd][(SIDES - 1) - s]
		[(DICE - 1) - pd][p];

	      if (tempt == -1) {
		fprintf(stderr, "Bug with mirror column tallies!\n");
		exit(1);
		/*continue;*/
	      }
	  ...
	  if ((PAIRSYM == 1) && ((s & 1) == 0)) {
	    ...
	    tempt = (*stc)[rows][(DICE - 1) - vd][s + 1]
	      [(DICE - 1) - pd][p];

	    if (tempt == -1) {
	      fprintf(stderr, "Bug with pair column tallies!\n");
	      exit(1);
	      /*continue;*/
	    }
	...
	if (PAIRSYM == 0) {
	  if (s == l) {
	    (*minst)[rows][rnum][s][p] = min[rnum];
	    (*maxst)[rows][rnum][s][p] = max[rnum];
	  }
	  else {
	    (*minst)[rows][rnum][s][p] = min[rnum] +
	      (*minst)[rows][rnum][s + 1][p];
	    (*maxst)[rows][rnum][s][p] = max[rnum] +
	      (*maxst)[rows][rnum][s + 1][p];
	  }
	}
	else {
	  if (s == (l - 1)) {
	    ...
	    (*minst)[rows][rnum][s][p] = min[rnum] +
	      (*minst)[rows][rnum][s + 2][p];
	    (*maxst)[rows][rnum][s][p] = max[rnum] +
	      (*maxst)[rows][rnum][s + 2][p];
	  }
	}
      }
      /*fprintf(stderr, "Bringing min,max for s=%d; p=%d to %lu, %lu\n",
...
void semwait(int n, __attribute__((__unused__)) int line) {
  ...
  if (DOFORK == 1) {
    ...
    if (semret == -1) {
      fprintf(stderr, "semdown failed at line %d\n", line);
      exit(1);
    }
  }
}
