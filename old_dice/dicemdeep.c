...
#define MIRRORSYM 0
#define MAXUNMIRROR 5
#define MINPERMDEPTH 8
...
int un_mir[DICE][SIDES];
int un_mir_cache[DICE][SIDES][DICE][SIDES];
...
void build_tallies_left(int (*)[DICE][SIDES],
			...
			uint64_t (*)[DICE][SIDES][DICE], int, int,
			int (*)[DICE][SIDES]);
...
void sum_find_row_mir(int (*g)[DICE][SIDES], uint64_t (*t)[DICE][DICE],
		      int row, int depth, int (*u)[DICE][SIDES], int c,
  ...
  /*fprintf(stderr, "at ctr=%lu; r=%d; d=%d\n", gctr++, row, depth);*/

  /* We gotta build our tallie left cache for this row */
  if ((depth == 0) && (row != DICE)) {
    build_tallies_left(g, &(min_tallie_left_cache[row][depth]),
		       &(max_tallie_left_cache[row][depth]), row, 0, u);
  }
  else {
    memcpy(&(min_tallie_left_cache[row][depth]), mint,
	   sizeof(min_tallie_left));
    memcpy(&(max_tallie_left_cache[row][depth]), maxt,
  ...
  if (depth < SIDES) {
	...
	return;
      ...
      if ((*t)[row][pl] + min_tallie_left_cache[row][depth][row][depth][pl] >
	  GOALTALLY) {
	/*fprintf(stderr, "Abandoning (over max) at depth %d, row %d\n",
	  depth, row);*/
	return;
  ...
  if (depth >= (SIDES)) {
	...
	fprintf(stderr, "\n");*/
      ...
      /*fprintf(stderr, "Moving on to row %d\n", row + 1);*/
      sum_find_row_mir(g, t, row + 1, 0, u, c,
		       &(min_tallie_left_cache[row][depth]),
		       &(max_tallie_left_cache[row][depth]));
      return;
    }
  ...
  /* Just assign this row a value if we're doing mirror symmetry */
  if (MIRRORSYM == 1) {
    if ((depth >= (SIDES / 2)) && ((*u)[row][depth] == 0)) {
      ...
      /* save this un mirror list */
      memcpy(&(un_mir_cache[row][depth]), u, sizeof(un_mir));

      sum_find_row_mir(&(g_cache_row[row][depth]), &(t_cache_row[row][depth]),
		       row, depth + 1, &(un_mir_cache[row][depth]), c,
		       &(min_tallie_left_cache[row][depth]),
		       &(max_tallie_left_cache[row][depth]));
  ...
  for (d = row; d < DICE; d++) {
    ...
    /*print_tallies(&(t_cache_row[row][depth]));*/

    if ((MIRRORSYM == 1) && (depth >= (SIDES / 2)) &&
	((*u)[row][depth] == 1)) {

      /* Skip this if it's the mirror */
      if (g_cache_row[row][depth][row][depth] ==
	  ((DICE * SIDES) - 1) - (*g)[row][(SIDES / 2) -
					   ((depth - (SIDES / 2)) + 1)]) {
	continue;
      }
    ...
    if ((MIRRORSYM == 1) && (c < MAXUNMIRROR) && (depth < (SIDES / 2)))  {
      ...
      /* recompute tallies left */
      build_tallies_left(&(g_cache_row[row][depth]),
			 &(min_tallie_left_cache[row][depth]),
			 &(max_tallie_left_cache[row][depth]), row, 0,
			 &(un_mir_cache[row][depth]));

      sum_find_row_mir(&(g_cache_row[row][depth]),
		       &(t_cache_row[row][depth]),
		       row, depth + 1, &(un_mir_cache[row][depth]), c + 1,
		       &(min_tallie_left_cache[row][depth]),
		       &(max_tallie_left_cache[row][depth]));

      /* restore the unmirror list */
      memcpy(&(un_mir_cache[row][depth]), u, sizeof(un_mir));
	...
	((*u)[row][depth] == 0)) {
      ...
      /* we're supposed to be mirrored here so we need to find the value
       * that we're supposed to be at and swap it into this position
       */
      found = 0;
...
void sum_find_perm(int (*g)[DICE][SIDES], uint64_t (*t)[DICE][DICE],
		   uint64_t (*p)[DICE + 1][DICEFACT], int depth) {

  int k, d, pl;
  int perm[DICE + 1];
  uint64_t pr_score, linear;
  int i, pc, plen;
  uint64_t pgoal;
  ...
  /* catch up if we haven't computed the perms yet */
  ...
  if (depth >= MINPERMDEPTH) {
    ...
    min_all_perms(p, &all_perms_min, depth - 1);

    for (plen = 2; plen <= DICE; plen++) {
      /*pc = npr(DICE, plen);*/
      pc = p_max_index[plen];
	...
	if (all_perms_min[plen][i] > pgoal) {
	  /*if (depth >= MINPERMDEPTH) {
	    fprintf(stderr, "Abandoning (over perm max) at depth %d\n", depth);
	    fprintf(stderr, "plen=%d; pcount=%lu; min=%lu; goal=%lu; i=%d\n",
		    plen, (*p)[plen][i], all_perms_min[plen][i], pgoal, i);
		    }*/
	  /*print_all_perms(&all_perms_min);*/
	  return;
	}
  ...
  if ((MIRRORSYM == 1) &&
    ...
    if (depth >= MINPERMDEPTH) {
      /* gotta figure out the new perms */
      memcpy(&(p_cache[depth]), p, sizeof(all_perms));
  ...
  if (depth > 0) {
    while ((k = next_perm(&(g_cache[depth]), &perm, k, depth)) != DICE) {
